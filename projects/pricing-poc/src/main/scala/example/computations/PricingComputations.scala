...
object PricingComputations {
  ...
  def computePricingForSqlDB()(df: DataFrame): DataFrame = {
    ...
    val computeDataFrame = pricingDataFrames
      .get("Compute")
      .getOrElse(throw new RuntimeException(s"Compute pricing data not found"))
    val storageDataFrame = loadPricingDataFrames(
      PlatformType.AzureSqlManagedInstance
    ).get("Storage")
      .getOrElse(throw new RuntimeException(s"Storage pricing data not found"))

    df.transform(PaaSPricing.transformPlatform())
      .transform(PaaSPricing.enrichWithStoragePricing(storageDataFrame))
      .transform(
        PaaSPricing.enrichWithReservedPricing(computeDataFrame, "1 Year")
      )
      .transform(
        PaaSPricing.enrichWithReservedPricing(computeDataFrame, "3 Years")
      )
  ...
  def computePricingForSqlMI()(df: DataFrame): DataFrame = {
    implicit val spark: SparkSession = df.sparkSession
    val pricingDataFrames = loadPricingDataFrames(
      PlatformType.AzureSqlManagedInstance
    )
    val computeDataFrame = pricingDataFrames
      .get("Compute")
      .getOrElse(throw new RuntimeException(s"Compute pricing data not found"))
    val storageDataFrame = pricingDataFrames
      .get("Storage")
      .getOrElse(throw new RuntimeException(s"Storage pricing data not found"))
  ...
  def computePricingForSqlVM()(df: DataFrame): DataFrame = {
    implicit val spark: SparkSession = df.sparkSession
    val pricingDataFrames = loadPricingDataFrames(
      PlatformType.AzureSqlVirtualMachine
    )
    val computeDataFrame = pricingDataFrames
      .get("Compute")
      .getOrElse(throw new RuntimeException(s"Compute pricing data not found"))
    ...
    df.transform(IaaSPricing.transformPlatform())
      ...
      .transform(
        IaaSPricing.enrichWithReservedPricing(computeDataFrame, "1 Year")
      )
      .transform(
        IaaSPricing.enrichWithReservedPricing(computeDataFrame, "3 Years")
      )
      .transform(
        IaaSPricing.enrichWithAspProdPricing(computeDataFrame, "1 Year")
      )
      .transform(
        IaaSPricing.enrichWithAspProdPricing(computeDataFrame, "3 Years")
      )
      .transform(
        IaaSPricing.enrichWithAspDevTestPricing(computeDataFrame, "1 Year")
      )
      .transform(
        IaaSPricing.enrichWithAspDevTestPricing(computeDataFrame, "3 Years")
      )
      .transform(IaaSPricing.addMonthlyCostOptions())
  }

  def loadPricingDataFrames(
    ...
    val pricingDataFolderPath = Paths
      .get(
