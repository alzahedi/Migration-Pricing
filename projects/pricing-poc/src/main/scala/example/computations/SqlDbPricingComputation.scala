...
class SqlDbPricingComputation extends PricingComputation {
  override def compute(df: DataFrame)(implicit spark: SparkSession): DataFrame = {
    val pricing = PricingDataLoader.load(PlatformType.AzureSqlDatabase)
    val computeDF = pricing.getOrElse("Compute", throw new RuntimeException("Compute pricing missing"))
    val storageDF = PricingDataLoader.load(PlatformType.AzureSqlManagedInstance).getOrElse("Storage", throw new RuntimeException("Storage pricing missing"))
